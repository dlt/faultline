...
RSpec.describe Faultline::Mcp::Tools::ResolveErrorGroup do
  describe ".mutates?" do
    it "is true" do
  ...
  describe ".call" do
    ...
    context "when mcp_readonly is false" do
      ...
      it "marks the group resolved and stamps resolved_at" do
        result = described_class.call(id: group.id)

        expect(result[:resolved]).to be true
        expect(result[:group][:status]).to eq("resolved")
        expect(group.reload.resolved_at).not_to be_nil
      end

      it "logs a note when provided" do
        expect(Rails.logger).to receive(:info).with(/note: investigated/)
        described_class.call(id: group.id, note: "investigated")
      end
